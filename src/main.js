...
async
function cdninfo() {
    fetch("/cdn-cgi/trace").then(a = >200 === a.status ? {
        status: "success",
        content: a
    }: {
        status: "failure",
        ...
        if ("success" === a.status) return a.content.text();
        ...
        for (let i in n) if ( - 1 != n[i].indexOf(t)) {
...
function networktest() {
    ...
    e = null,
    n = null;
    fetch("/cache-check").then(a = >{
        if (200 === a.status) return a;
        throw "Can't get response"
    }).then(a = >(e = (Math.round(100 * performance.now()) / 100 / 1e3).toFixed(2), a)).then(t = >(n = (e - a).toFixed(2), {
        time: (e - a).toFixed(2),
...
function getdns() {
    ...
    await fetch("https://dns.alidns.com/resolve?name=cf.186526.xyz&type=1").then(a = >{
        if (200 === a.status) return a;
        throw "Can't fetch DNS resolve"
    }).then(a = >a.json()).then(a = >{
        let e = create("ul");
        for (let n in a.Answer) b = create("li"),
    ...
    await fetch("https://dns.alidns.com/resolve?name=cf.186526.xyz&type=28").then(a = >{
        if (200 === a.status) return a;
        throw "Can't fetch DNS resolve"
    }).then(a = >a.json()).then(a = >{
        let e = create("ul");
